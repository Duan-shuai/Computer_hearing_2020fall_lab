...
class code_four:
    ...
    def quantification_a(self, d, a):
        ...
        # 量化因子法相当于整体平移，由（-7，7）->(0，15)
        return final_code

    def quantification_log(self, d, a):
        c = math.log(abs(d) + 1)
        if c > 7:
            c = 7
        if d < 0:
    ...
    def code(self, data, a, flag):
        ...
        for i in range(length - 1):
            ...
            if flag == 0:
                ...
                decode[i + 1] = decode[i] + (code[i + 1] - 8) * a  # 解码，用于反馈
            # 对数变换法
            else:
                code[i + 1] = self.quantification_log(code[i + 1], a)
                if code[i+1] >= 8:   # 负数
                    decode[i + 1] = decode[i] - math.exp(code[i + 1] - 8) + 1  # 解码，用于反馈
                else:
